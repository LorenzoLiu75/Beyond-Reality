<?php
  ...
  if (isset($_GET['id'])) {
    $idJabatan = mysqli_real_escape_string($conn, $_GET["id"]);
  ...
  if (isset($_POST['updateJabatan'])) {
    $sql = "UPDATE jabatan SET
            nama_jabatan = '$_POST[namaJabatan]'
            WHERE id_jabatan = '{$idJabatan}'";
    $result = mysqli_query($conn, $sql);

    if($result) {
      $msg = "<div class='alert alert-success'>Jabatan is updated</div>";
      header("refresh:2; url=dashboard.php");
    }
    else {
      $msg = "<div class='alert alert-danger'>Jabatan is not updated</div>";
    }
  }
